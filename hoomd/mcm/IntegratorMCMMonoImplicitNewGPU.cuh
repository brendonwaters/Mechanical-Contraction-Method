...
#ifndef _MCM_IMPLICIT_NEW_CUH_
#define _MCM_IMPLICIT_NEW_CUH_
...
struct mcm_implicit_args_new_t
    {
    //! Construct a mcm_implicit_args_new_t
    mcm_implicit_args_new_t(Scalar4 *_d_postype,
...
cudaError_t gpu_mcm_insert_depletants_queue(const mcm_implicit_args_new_t &args, const typename Shape::param_type *d_params);

template< class Shape >
cudaError_t gpu_mcm_implicit_accept_reject_new(const mcm_implicit_args_new_t &args, const typename Shape::param_type *d_params);
...
template< class Shape >
__global__ void gpu_mcm_insert_depletants_queue_kernel(Scalar4 *d_postype,
...
//! Kernel driver for gpu_update_mcm_kernel()
/*! \param args Bundled arguments
    ...
    This templatized method is the kernel driver for MCM update of any shape. It is instantiated for every shape at the
    bottom of this file.

    \ingroup mcm_kernels
*/
template< class Shape >
cudaError_t gpu_mcm_insert_depletants_queue(const mcm_implicit_args_new_t& args, const typename Shape::param_type *params)
    {
    ...
    if (max_block_size == -1)
        {
        cudaFuncGetAttributes(&attr, gpu_mcm_insert_depletants_queue_kernel<Shape>);
    ...
    if (min_shared_bytes >= args.devprop.sharedMemPerBlock)
        throw std::runtime_error("Insufficient shared memory for MCM kernel: reduce number of particle types or size of shape parameters");

    while (shared_bytes + attr.sharedSizeBytes >= args.devprop.sharedMemPerBlock)
        {
        block_size -= args.devprop.warpSize;
        if (block_size == 0)
            throw std::runtime_error("Insufficient shared memory for MCM kernel");
    ...
    gpu_mcm_insert_depletants_queue_kernel<Shape><<<grid, threads, shared_bytes, args.stream>>>(args.d_postype,
                                                                 args.d_orientation,
                                                                 args.d_counters,
                                                                 args.d_cell_idx,
                                                                 args.d_cell_size,
...
//! Kernel driver for gpu_mcm_implict_accept_reject_new_kernel()
/*! \param args Bundled arguments
    \param d_params Per-type shape parameters
    \returns Error codes generated by any CUDA calls, or cudaSuccess when there is no error
