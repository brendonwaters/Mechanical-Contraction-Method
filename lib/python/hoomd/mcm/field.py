...
""" Apply external fields to MCM simulations.
"""
...
# to the mcm integrator. While external fields are Compute types and are added
...
class lattice_field(_external):
    ...
    * **lattice_num_samples** -- number of samples used to compute the average and standard deviation
    ...
    Example::
        ...
        mcm.field.lattice_field(mc=mc, position=fcc_lattice, k=1000.0);
    ...
    def __init__(self, mc, position = [], orientation = [], k = 0.0, q = 0.0, symmetry = [], composite=False):
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if isinstance(mc, integrate.sphere):
                cls = _mcm.ExternalFieldLatticeSphere;
            elif isinstance(mc, integrate.convex_polygon):
                cls = _mcm.ExternalFieldLatticeConvexPolygon;
            elif isinstance(mc, integrate.simple_polygon):
                cls = _mcm.ExternalFieldLatticeSimplePolygon;
            elif isinstance(mc, integrate.convex_polyhedron):
                cls = _mcm.ExternalFieldLatticeConvexPolyhedron;
            elif isinstance(mc, integrate.convex_spheropolyhedron):
                cls = _mcm.ExternalFieldLatticeSpheropolyhedron;
            elif isinstance(mc, integrate.ellipsoid):
                cls = _mcm.ExternalFieldLatticeEllipsoid;
            elif isinstance(mc, integrate.convex_spheropolygon):
                cls =_mcm.ExternalFieldLatticeSpheropolygon;
            elif isinstance(mc, integrate.faceted_sphere):
                cls =_mcm.ExternalFieldLatticeFacetedSphere;
            elif isinstance(mc, integrate.polyhedron):
                cls =_mcm.ExternalFieldLatticePolyhedron;
            elif isinstance(mc, integrate.sphinx):
                cls =_mcm.ExternalFieldLatticeSphinx;
            elif isinstance(mc, integrate.sphere_union):
                cls = _mcm.ExternalFieldLatticeSphereUnion;
            elif isinstance(mc, integrate.convex_polyhedron_union):
                cls = _mcm.ExternalFieldLatticeConvexPolyhedronUnion;
            else:
        ...
        enlist = hoomd.mcm.data._param.ensure_list;
    ...
    def set_references(self, position = [], orientation = []):
        ...
        Example::
            ...
            mc = mcm.integrate.sphere(seed=415236);
            lattice = mcm.field.lattice_field(mc=mc, position=fcc_lattice, k=1000.0);
    ...
    def get_average_energy(self):
        ...
        Example::
            mc = mcm.integrate.sphere(seed=415236);
            lattice = mcm.field.lattice_field(mc=mc, position=fcc_lattice, k=exp(15));
            run(20000)
...
class external_field_composite(_external):
    ...
    Args:
        mc (:py:mod:`hoomd.mcm.integrate`): MC integrator (don't specify a new integrator later, external_field_composite will continue to use the old one)
    ...
    Examples::
        ...
        walls = mcm.compute.walls(...)
        lattice = mcm.compute.lattice(...)
        composite_field = mcm.compute.external_field_composite(mc, fields=[walls, lattice])
    ...
    def __init__(self, mc, fields = None):
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if isinstance(mc, integrate.sphere):
                cls = _mcm.ExternalFieldCompositeSphere;
            elif isinstance(mc, integrate.convex_polygon):
                cls = _mcm.ExternalFieldCompositeConvexPolygon;
            elif isinstance(mc, integrate.simple_polygon):
                cls = _mcm.ExternalFieldCompositeSimplePolygon;
            elif isinstance(mc, integrate.convex_polyhedron):
                cls = _mcm.ExternalFieldCompositeConvexPolyhedron;
            elif isinstance(mc, integrate.convex_spheropolyhedron):
                cls = _mcm.ExternalFieldCompositeSpheropolyhedron;
            elif isinstance(mc, integrate.ellipsoid):
                cls = _mcm.ExternalFieldCompositeEllipsoid;
            elif isinstance(mc, integrate.convex_spheropolygon):
                cls =_mcm.ExternalFieldCompositeSpheropolygon;
            elif isinstance(mc, integrate.faceted_sphere):
                cls =_mcm.ExternalFieldCompositeFacetedSphere;
            elif isinstance(mc, integrate.polyhedron):
                cls =_mcm.ExternalFieldCompositePolyhedron;
            elif isinstance(mc, integrate.sphinx):
                cls =_mcm.ExternalFieldCompositeSphinx;
            elif isinstance(mc, integrate.sphere_union):
                cls = _mcm.ExternalFieldCompositeSphereUnion;
            elif isinstance(mc, integrate.convex_polyhedron_union):
                cls = _mcm.ExternalFieldCompositeConvexPolyhedronUnion;
            else:
                hoomd.context.msg.error("compute.position_lattice_field: Unsupported integrator.\n");
                raise RuntimeError("Error initializing compute.position_lattice_field");
        else:
    ...
    def add_field(self, fields):
        ...
        Example::
            ...
            mc = mcm.integrate.shape(...);
            composite_field = mcm.compute.external_field_composite(mc)
            walls = mcm.compute.walls(..., setup=False)
            lattice = mcm.compute.lattice(..., setup=False)
...
class wall(_external):
    ...
    * **mcm_wall_volume** : the volume associated with the intersection of implemented walls. This number is only meaningful
    ...
    * **mcm_wall_sph_rsq-i** : the squared radius of the spherical wall indexed by i, beginning at 0 in the order the sphere
      walls were added to the system.
    * **mcm_wall_cyl_rsq-i** : the squared radius of the cylindrical wall indexed by i, beginning at 0 in the order the
    ...
    Example::
        ...
        log = analyze.log(quantities=['mcm_wall_volume','mcm_wall_sph_rsq-0'], period=100, filename='log.dat', overwrite=True);
    ...
    def __init__(self, mc, composite=False):
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if isinstance(mc, integrate.sphere):
                ...
                cls = _mcm.WallConvexPolyhedron;
            elif isinstance(mc, integrate.convex_spheropolyhedron):
                cls = _mcm.WallSpheropolyhedron;
            else:
    ...
    def add_sphere_wall(self, radius, origin, inside = True):
        ...
        """
        hoomd.util.print_status_line();
        self.cpp_compute.AddSphereWall(_mcm.make_sphere_wall(radius, origin, inside));
    ...
    def set_sphere_wall(self, index, radius, origin, inside = True):
        ...
        """
        hoomd.util.print_status_line();
        self.cpp_compute.SetSphereWallParameter(index, _mcm.make_sphere_wall(radius, origin, inside));
    ...
    def add_cylinder_wall(self, radius, origin, orientation, inside = True):
        ...
        Args:
            ...
            orientation (tuple): vector that defines the direction of the long axis of the cylinder. will be normalized automatically by mcm.
        ...
        hoomd.util.print_status_line();
        param = _mcm.make_cylinder_wall(radius, origin, orientation, inside);
    ...
    def set_cylinder_wall(self, index, radius, origin, orientation, inside = True):
        ...
        Args:
            ...
            orientation (tuple): New vector that defines the direction of the long axis of the cylinder. will be normalized automatically by mcm.
    ...
    def add_plane_wall(self, normal, origin):
        ...
        Args:
            normal (tuple): vector normal to the plane. this, in combination with a point on the plane, defines the plane entirely. It will be normalized automatically by mcm.
        ...
        """
        hoomd.util.print_status_line();
        self.cpp_compute.AddPlaneWall(_mcm.make_plane_wall(normal, origin, True));
    ...
    def set_plane_wall(self, index, normal, origin):
        ...
        Args:
            ...
            normal (tuple): new vector normal to the plane. this, in combination with a point on the plane, defines the plane entirely. It will be normalized automatically by mcm.
                            The direction of the normal vector defines the confinement condition associated with the plane wall. If every part of a particle exists in the halfspace into which the normal points, then that particle is CONFINED by the plane wall.
        ...
        """
        hoomd.util.print_status_line();
        self.cpp_compute.SetPlaneWallParameter(index, _mcm.make_plane_wall(normal, origin, True));
...
class frenkel_ladd_energy(_compute):
    ...
    and :py:class:`hoomd.mcm.update.remove_drift`.
    ...
    Example::
        ...
        fl = mcm.compute.frenkel_ladd_energy(mc=mc, ln_gamma=0.0, q_factor=10.0, r0=rs, q0=qs, drift_period=1000)
    ...
    def __init__(   self,
        ...
        self.lattice = lattice_field(   self.mc,
                                        ...
                                        symmetry=symmetry);
        self.remove_drift = hoomd.mcm.update.remove_drift(self.mc, self.lattice, period=drift_period);
...
class callback(_external):
    ...
    Example::
          ...
          mcm.field.callback(mc=mc, energy_function=energy);
          run(100)
    """
    def __init__(self, mc, energy_function, composite=False):
        ...
        if not hoomd.context.exec_conf.isCUDAEnabled():
            if isinstance(mc, integrate.sphere):
                cls = _mcm.ExternalCallbackSphere;
            elif isinstance(mc, integrate.convex_polygon):
                cls = _mcm.ExternalCallbackConvexPolygon;
            elif isinstance(mc, integrate.simple_polygon):
                cls = _mcm.ExternalCallbackSimplePolygon;
            elif isinstance(mc, integrate.convex_polyhedron):
                cls = _mcm.ExternalCallbackConvexPolyhedron;
            elif isinstance(mc, integrate.convex_spheropolyhedron):
                cls = _mcm.ExternalCallbackSpheropolyhedron;
            elif isinstance(mc, integrate.ellipsoid):
                cls = _mcm.ExternalCallbackEllipsoid;
            elif isinstance(mc, integrate.convex_spheropolygon):
                cls =_mcm.ExternalCallbackSpheropolygon;
            elif isinstance(mc, integrate.faceted_sphere):
                cls =_mcm.ExternalCallbackFacetedSphere;
            elif isinstance(mc, integrate.polyhedron):
                cls =_mcm.ExternalCallbackPolyhedron;
            elif isinstance(mc, integrate.sphinx):
                cls =_mcm.ExternalCallbackSphinx;
            elif isinstance(mc, integrate.sphere_union):
                cls = _mcm.ExternalCallbackSphereUnion;
            else:
                hoomd.context.msg.error("mcm.field.callback: Unsupported integrator.\n");
                raise RuntimeError("Error initializing python callback");
        else:
            hoomd.context.msg.error("GPU not supported")
            raise RuntimeError("Error initializing mcm.field.callback");
